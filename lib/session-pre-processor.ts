...
export async function extractSessionSummary(
  ...
  try {
    packets = await withConcurrency(sorted, PACKET_CONCURRENCY, async ({ key, idx }) => {
      try {
  ...
  const scrollDepth = sawScrollEvent
    ? clamp(maxScrollY / docHeight, 0, 1)
    : 0
