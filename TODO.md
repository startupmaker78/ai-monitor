...
- **[UX релевантности] укоротить путь в релевантность-строке** — сейчас печатаем
  полный `ym:pv:URLPathFull` (`/kabinet/lchinyye-kabinety-dlya-internet-magazinov`),
  длинно. НЕ обрезать многоточием в середине (путь станет неузнаваемым). Показывать
  последний сегмент или заголовок страницы, если он у нас есть (напр. name цели/
  таргета). Клиент должен понять, о какой странице речь.
