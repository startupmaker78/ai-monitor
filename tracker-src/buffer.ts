...
export class EventBuffer {
  ...
  add(event: unknown): void {
    ...
    if (e.type === 2) {
      ...
      // несжатым. Изолируем снапшот в свой пакет:
      ...
      // Зачем изоляция: тело одного пакета на collect ограничено
      // серверным MAX_WIRE_BYTES=3 MiB (YC-платформа режет тело >3.5 MiB
      // на проводе ДО контейнера с 413); изоляция держит тело ≈ размер
      // самого снапшота. Сейчас клиент шлёт пакеты gzip'ом (см.
      // transport.ts) → сжатый снапшот укладывается с большим запасом,
      // так что изоляция технически избыточна, но оставлена НАМЕРЕННО:
      // снапшот уходит немедленно отдельным пакетом → короткие сессии
      // воспроизводимы (DECISIONS 2026-07-09).
