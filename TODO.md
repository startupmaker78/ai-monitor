...
- **[B, не критично] ротация site token `e464…`** — теперь БОЕВОЙ:
  вшит в `<script>` на academy → ротация = новый токен + обновление
  кода на academy (координация с владельцем), не просто смена.
