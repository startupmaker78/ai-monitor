...
// Клиентский таймаут запроса анализа. Было 150с при норме ~55с; с подъёмом
// MAX_SESSIONS_PER_ANALYSIS до 90 расчётное время анализа ~150с, то есть
// прежний таймаут стал СЕДЬМЫМ потолком раньше всех остальных и рубил бы
// нормальный прогон. 280с — впритык под Gateway execution_timeout=300с, с
// запасом ~20с на сетевые накладные и TLS, чтобы клиент отваливался ПЕРВЫМ
// с внятным сообщением, а не ловил 504 от Gateway.
// Следующий потолок — уже сам Gateway (300с, ~230 сессий); обойти его можно
// только вызовом контейнера напрямую или асинхронной схемой, то есть
// переделкой, а не константой.
// При аборте НЕ говорим «провал»: сервер мог довести анализ (свои гарды),
// результат появится в /recommendations — сообщение «идёт дольше,
// проверьте позже».
