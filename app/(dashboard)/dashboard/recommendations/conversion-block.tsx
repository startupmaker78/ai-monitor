...
// Выбранный анализ прогнан РАНЬШЕ, чем к странице привязали целевое действие
// (первая привязка null → цель разрешена и на странице с готовыми анализами —
// см. гейт в targets/actions.ts). Конверсия по этой цели к выводам такого
// анализа не относится, и показывать её над ним = ретроактивно переподписать
// чужой целью. Сам анализ и рекомендации не трогаем: скрываем только цифру.
// Имя цели в текст НЕ подставляем намеренно — иначе снова получится подпись
// цели под анализом, который её не видел.
export function ConversionPredatesGoal() {
  return (
    <Shell>
      <p className="text-sm text-muted-foreground">
        Этот анализ выполнен до того, как было задано целевое действие —
        конверсия по нему к его выводам не относится.
      </p>
    </Shell>
  )
}
